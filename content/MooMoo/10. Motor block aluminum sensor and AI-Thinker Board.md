...
```
// ============================
// Small Enclosure w/ Sensor & Cable Holes
// ============================
...
// ---- Cable holes ----
CABLE_SIZE     = [10,3,3];   // h,r,r (cylinder)
CABLE_Z        = -8;
CABLE_X_OFF    = 4.5;

// ---- Sensor notch (ring block added) ----
SENSOR_POS     = [10,0,-11.5];
SENSOR_OUT     = [6,6,2];
SENSOR_IN_EDGE = 2.9;        // scalar cube => centered small cube
...
module cable_holes() {
  translate([ CABLE_X_OFF, 0, CABLE_Z])
    cylinder(h=CABLE_SIZE[0], r=CABLE_SIZE[1], center=true);
  translate([-CABLE_X_OFF, 0, CABLE_Z])
    cylinder(h=CABLE_SIZE[0], r=CABLE_SIZE[2], center=true);
}

module sensor_notch() {
  difference() {
    translate(SENSOR_POS) cube(SENSOR_OUT, center=true);
    translate(SENSOR_POS) cube(SENSOR_IN_EDGE, center=true);
  }
}
...
module grip_dots() {
  for (p = DOT_POS) translate(p) sphere(r=DOT_R);
}

// ==================== Assembly ====================

module small_enclosure_assembly() {
  // body with cable holes removed
  difference() {
    main_body();
    cable_holes();
  }
